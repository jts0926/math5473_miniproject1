...
def objective(X_1, g_2, X_2, g_3, X_3):
    ...
    return norm(X_1,'fro')**2 + g_2*norm(hstack(X_2),1) + g_3*norm(X_3,'nuc')


def rpcaADMM(data, g2_coef=0.15, g3_coef=0.15):
    """
    ...
    if k > MAX_ITER-2 and (h['r_norm'][k] > h['eps_pri'][k]) or (h['s_norm'][k] > h['eps_dual'][k]):
    ...
    print('Finished Time: ', h['admm_toc'])
